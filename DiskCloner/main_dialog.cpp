...
void MainDialog::TimerStart(LARGE_INTEGER* pSpec)
{
    QueryPerformanceCounter(pSpec);
}

double MainDialog::TimerQuery(LARGE_INTEGER* pSpec)
{
...
void MainDialog::UpdateDisks()
{
  ...
  for (const auto& disk : _disks)
  {
    ...
    // Set multi part drive as default index if found!
    if (disk.letters.size() > 1) {
        MultiDriveIndex = idx;
    }
  }
  if (MultiDriveIndex != -1) {
      ComboBox_SetCurSel(combo, MultiDriveIndex);
  }
  Button_SetCheck(GetCtl(IDC_OPERATION_SAVE), BST_CHECKED);
...
#define LOCALIZE(x) SetDlgItemTextFromResourceString(_handle, IDC_ ## x, IDS_DLG_ ## x)
  ...
  SetDlgItemTextFromResourceString(_handle, IDC_LICENSE_BTN, IDC_LICENSE_CHECK);
#undef LOCALIZE
}
...
INT_PTR MainDialog::DlgProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  ...
  case WM_FLASH_STARTED:
    ...
    SetDlgItemText(_handle, IDC_STATIC_PROGRESS, TEXT("Starte Aktion..."));
    TimerStart(&start);
  ...
  case WM_FLASH_PROGRESS:
    if (wParam != message_magic_wparam)
      return FALSE;
    nPercentage = ((double)lParam/(double)MAXLONG)*100.0;
    nElapsed = TimerQuery(&start);
    nTotalTime = (100.0 / nPercentage) * nElapsed;
    nRemaningSeconds = nTotalTime - nElapsed; // Sekunden
    nRemaningMinutes = (int)(nRemaningSeconds / 60);
    nRemaningSeconds = (nTotalTime - nElapsed) - 60 * nRemaningMinutes;
    if ((int)nRemaningSeconds < 0) {
        nRemaningSeconds = 0;
    }
    nRemaningSeconds = round(nRemaningSeconds / 10.0) * 10.0;
    if ((int)round(nPercentage) >= 5) {
        ProgressMessage.Format(_T("%3d%% fertig, ca. %02d:%02d verbleibend"), (int)round(nPercentage), (int)nRemaningMinutes, (int)nRemaningSeconds);
    } else {
        ProgressMessage.Format(_T("%3d%% fertig, Berechne verbleibende Restzeit..."), (int)round(nPercentage));
    }
    SetDlgItemText(_handle, IDC_STATIC_PROGRESS, ProgressMessage);
  ...
  case WM_FLASH_FINISHED:
  {
    ...
    SetDlgItemText(_handle, IDC_STATIC_PROGRESS, TEXT("Fertig"));
